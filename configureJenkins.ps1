...
function Start-Jenkins($url, $header) {
	net start Jenkins | Out-File -Append logs\jenkins-config.log
	...
	if ($JenkinsServiceStatus.Status -ne 'Running') {
		...
		copy "C:\Program Files (x86)\Jenkins\jenkins.err.log" logs\jenkins.err.log | Out-File -Append logs\jenkins-config.log	
		Exit
	}
	...
	do {
		try {
			[bool] $notready = 0
			Invoke-WebRequest -Uri $url -Headers $header 2>&1 | Out-File -Append logs\jenkins-config.log
		}
		catch {
			[bool] $notready = 1
			Write-Host "Ожидание старта Jenkins..." -ForegroundColor Yellow
			$_ | Out-File -Append logs\jenkins-config.log
			Start-Sleep 10
		}
		if ($timer -eq 5) {
...
if ($step -eq 1 -or $step -eq '') {
	...
	Start-Sleep 3
	Write-Host 'Проверяю состояние Jenkins...' -ForegroundColor Yellow
	...
	while (-Not [System.IO.File]::Exists("C:\Program Files (x86)\Jenkins\secrets\initialAdminPassword")) {
		# разбудить Jenkins
		try {
			Invoke-WebRequest -Uri "http://localhost:8080/login?from=%2F" 2>&1 | Out-File -Append logs\jenkins-config.log
		}
		catch {
			Write-Host "Ожидание старта Jenkins..." -ForegroundColor Yellow
			$_ | Out-File -Append logs\jenkins-config.log
...
if ($step -eq 2) {
	...
	if ($tmp) {
		...
		$c_aiee_creds | Out-File -Append logs\jenkins-config.log
		Exit
	}
	...
	# извлекаем строку запуска jenkins агента и записываем её в agent.xml
	...
	$agentrun = Get-Content config\agent.xml -Raw
	if ($found) {
		$agentrun -ireplace '(<arguments>)(.*)',"<arguments>$($matches[0].Substring(5))" | Set-Content -Path C:\TOOLS\agent.xml
		...
		$tmp | Out-File -Append logs\jenkins-config.log
		Exit
	}
}