...
if ($step -eq $null) {
	...
	if (-Not [System.IO.File]::Exists("C:\Program Files (x86)\Jenkins\secrets\initialAdminPassword")) {
		# разбудить Jenkins
		try {
			Invoke-WebRequest -Uri "http://localhost:8080/login?from=%2F" 2>&1>$null
		}
		catch {
	...
	copy jenkins.xml "C:\Program Files (x86)\Jenkins\jenkins.xml" >>logs\install.log
	copy unsecure-config.xml "C:\Program Files (x86)\Jenkins\config.xml" >>logs\install.log
	xcopy nodes "C:\Program Files (x86)\Jenkins\nodes\" /E /Y >>logs\install.log
	copy C:\TOOLS\certs\INT\out\01\private.jks "C:\Program Files (x86)\Jenkins\secrets\private.jks" >>logs\install.log
	copy jenkins.model.JenkinsLocationConfiguration.xml "C:\Program Files (x86)\Jenkins\jenkins.model.JenkinsLocationConfiguration.xml" >>logs\install.log
	copy com.ptsecurity.appsec.ai.ee.utils.ci.integration.plugin.jenkins.Plugin.xml "C:\Program Files (x86)\Jenkins\com.ptsecurity.appsec.ai.ee.utils.ci.integration.plugin.jenkins.Plugin.xml" >>logs\install.log
	...
	net stop Jenkins >>logs\install.log
	net start Jenkins >>logs\install.log
	...
	if ($JenkinsServiceStatus.Status -ne 'Running') {
		...
		copy "C:\Program Files (x86)\Jenkins\jenkins.err.log" logs\jenkins.err.log >>logs\install.log	
		Exit
	}
	...
	# разбудить Jenkins
	try {
		Invoke-WebRequest -Uri "https://$($myFQDN):8080/" 2>&1>$null
	}
	catch {
		Write-Host "Ожидание старта Jenkins..." -ForegroundColor Yellow	
	}
	...
	-Method "POST" `
	-ContentType "application/x-www-form-urlencoded" `
	-Body "username=svc_ptai&password1=P%40ssw0rd&password2=P%40ssw0rd&fullname=svc_ptai&email=svc_ptai%40mail.ru&json=%7B%22username%22%3A+%22svc_ptai%22%2C+%22password1%22%3A+%22P%40ssw0rd%22%2C+%22%24redact%22%3A+%5B%22password1%22%2C+%22password2%22%5D%2C+%22password2%22%3A+%22P%40ssw0rd%22%2C+%22fullname%22%3A+%22svc_ptai%22%2C+%22email%22%3A+%22svc_ptai%40mail.ru%22%7D&Submit=%D0%A1%D0%BE%D0%B7%D0%B4%D0%B0%D1%82%D1%8C+%D0%BF%D0%BE%D0%BB%D1%8C%D0%B7%D0%BE%D0%B2%D0%B0%D1%82%D0%B5%D0%BB%D1%8F" >>logs\install.log
...
if ($step -eq 2) {
	...
	if ($tmp) {
		...
		echo $c_aiee_creds >>logs\install.log
		Exit
	}
	...
	copy secure-config.xml "C:\Program Files (x86)\Jenkins\config.xml" >>logs\install.log
	net stop Jenkins >>logs\install.log
	net start Jenkins >>logs\install.log
	# ждём пока сервис поднимется и (потенциально) упадёт
	Start-Sleep 10
	...
	$JenkinsServiceStatus = Get-Service Jenkins -ErrorAction Stop
	if ($JenkinsServiceStatus.Status -ne 'Running') {
		Write-Host 'Ошибка: Служба Jenkins не запущена. Логи скопированы в папку logs. Пожалуйста, устраните ошибку, удалите Jenkins и перезапустите установку с шага 7.' -ForegroundColor Red
		copy "C:\Program Files (x86)\Jenkins\jenkins.err.log" logs\jenkins.err.log >>logs\install.log	
		Exit
	}
